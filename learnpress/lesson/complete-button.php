...
<?php } else { ?>
	...
	<?php if ( !LP()->user->has( 'finished-course', $course->id ) && LP()->user->has( 'enrolled-course', $course->id ) ) { ?>
		...
		<button class="complete-lesson-button" data-id="<?php print_r( $lesson->id );?>" data-nonce="<?php echo wp_create_nonce( 'learn-press-complete-lesson-' . $lesson->id );?>"><?php _e( 'Complete Lesson', 'education-lite' );?></button>
